...
eval_14b.py
-----------
Evaluation script for cipher-instruct-14B-20260321_143806.
Evaluates base model vs fine-tuned (via disable_adapter).

Metrics: Valid JSON, Algo correct, Output exact, Cipher type, Key accuracy.
Breakdown: split (seen/unseen), cipher type, language, task type.

Usage:
  python eval_14b.py                          # uses defaults
...
DEFAULT_ADAPTER = "checkpoints/cipher-instruct-14B-20260321_143806/final_adapter"
DEFAULT_DATA    = "data/eval300.json"
...
# ─────────────────────────────────────────────
# MODEL LOADING (QLoRA — same as training)
# ─────────────────────────────────────────────
...
def parse_json_output(raw: str):
    """Parse JSON from model output. Returns (dict|None, bool)."""
    try:
        clean = raw.strip()
        if clean.startswith("```"):
            parts = clean.split("```")
...
def verify_algo(pred: Dict, gt: Dict, lang: str) -> bool:
    """Cryptographic check: apply predicted key and compare output."""
    try:
...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluate cipher-instruct-14B model (base vs fine-tuned)."
    )
    ap.add_argument("--base",           default=DEFAULT_BASE,
                    help=f"Base model name or path (default: {DEFAULT_BASE})")
    ap.add_argument("--adapter",        default=DEFAULT_ADAPTER,
                    help=f"LoRA adapter path (default: {DEFAULT_ADAPTER})")
    ap.add_argument("--data",           default=DEFAULT_DATA,
                    help=f"Eval dataset JSON (default: {DEFAULT_DATA})")
    ...
    log_path = os.path.join("logs", f"eval_{MODEL_TAG}_{ts}.out")
    ...
    print("=" * 80)
    print(f"CIPHERCHAT EVALUATION — {MODEL_TAG.upper()} | base vs fine-tuned | seen vs unseen")
    print("=" * 80)
    ...
    out_path = os.path.join(args.out_dir, f"eval_{MODEL_TAG}_{ts}.json")
